 ...
 *
 * @param rat Radio access technology. Only SIM7000_RAT_LTE_NB1 and SIM7000_RAT_LTE_M1 are valid.
 * @param bands Band bitmap.
 */
static int configure_bands(enum sim7000_rat rat, uint32_t bands)
{
	const char *band_cmd;
	uint32_t used_bands = 0;

	if (rat == SIM7000_RAT_LTE_NB1) {
		band_cmd = "AT+CBANDCFG=\"NB-IOT\"";
		used_bands = bands & SIM7000_LTE_CHANNEL_MASK_NB1;
	} else if (rat == SIM7000_RAT_LTE_M1) {
		band_cmd = "AT+CBANDCFG=\"CAT-M\"";
		used_bands = bands & SIM7000_LTE_CHANNEL_MASK_M1;
	} else {
		return -EINVAL;
	}
	...
	/* Send list of bands */
	for (uint8_t i = 0; i < SIM7000_NUM_LTE_CHANNELS; i++) {
...
static int configure_radio(void)
{
	...
	if (mdata.radio.rat < SIM7000_RAT_LTE_NB1 || mdata.radio.rat > SIM7000_RAT_GSM) {
		return -EINVAL;
	}
	...
	if (mdata.radio.rat == SIM7000_RAT_GSM) {
		mode_cmd = "AT+CNMP=13";
	} else {
		mode_cmd = "AT+CNMP=38";
	}
	...
	if (mdata.radio.rat == SIM7000_RAT_LTE_NB1 || mdata.radio.rat == SIM7000_RAT_LTE_AUTO) {
		ret = configure_bands(SIM7000_RAT_LTE_NB1, mdata.radio.lte_bands_nb1);
		if (ret != 0) {
	...
	if (mdata.radio.rat == SIM7000_RAT_LTE_M1 || mdata.radio.rat == SIM7000_RAT_LTE_AUTO) {
		ret = configure_bands(SIM7000_RAT_LTE_M1, mdata.radio.lte_bands_m1);
		if (ret != 0) {
...
int sim7000_pdp_activate(void)
{
	...
	const char *buf = mdata.radio.rat == SIM7000_RAT_GSM ? "AT+CREG?" : "AT+CEREG?";
	struct modem_cmd cmds[] = {
		MODEM_CMD(mdata.radio.rat == SIM7000_RAT_GSM ? "+CREG: " : "+CEREG: ", on_cmd_cereg,
			  2U, ",")};
...
{
	if (rat < SIM7000_RAT_LTE_NB1 || rat > SIM7000_RAT_GSM) {
		return -EINVAL;
	}
...
{
	if (nb1 != 0 && (nb1 & ~SIM7000_LTE_CHANNEL_MASK_NB1) != 0) {
		LOG_ERR("Illegal NB-IoT band selection");
		return -EINVAL;
	}

	if (m1 != 0 && (m1 & ~SIM7000_LTE_CHANNEL_MASK_M1) != 0) {
