...
static void modem_pwrkey(void)
{
	...
	/* Power pin should be high for 2 seconds. */
...
static int modem_setup(void)
{
	...
	/*
	 * DFRobot/SimCom ship several regional SIM7000 variants (A/C/E/G, ...)
	 * that all answer AT+CGMM with "SIMCOM_SIM7000<variant>". Only check the
	 * common prefix.
	 */
	if (strncmp(mdata.mdm_model, "SIMCOM_SIM7000", strlen("SIMCOM_SIM7000")) != 0) {
		LOG_ERR("Wrong modem model: %s", mdata.mdm_model);
		ret = -EINVAL;
		return ret;
	}

	return sim7000_pdp_activate();
}
...
#if IS_ENABLED(CONFIG_MODEM_SIMCOM_SIM7000_RAT_NB1)
	mdata.radio.rat = SIM7000_RAT_LTE_NB1;
#elif IS_ENABLED(CONFIG_MODEM_SIMCOM_SIM7000_RAT_M1)
	mdata.radio.rat = SIM7000_RAT_LTE_M1;
#elif IS_ENABLED(CONFIG_MODEM_SIMCOM_SIM7000_RAT_LTE_AUTO)
	mdata.radio.rat = SIM7000_RAT_LTE_AUTO;
#elif IS_ENABLED(CONFIG_MODEM_SIMCOM_SIM7000_RAT_GSM)
	mdata.radio.rat = SIM7000_RAT_GSM;
#else
#error "Invalid RAT choice"
#endif
	mdata.radio.lte_bands_nb1 = CONFIG_MODEM_SIMCOM_SIM7000_LTE_BANDS_NB1;
	mdata.radio.lte_bands_m1 = CONFIG_MODEM_SIMCOM_SIM7000_LTE_BANDS_M1;
