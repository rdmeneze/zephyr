/*
 * Copyright (C) 2023 Rafael Dias Menezes <rdmeneze@gmail.com>
 *
...
#include "simcom-sim7000.h"

#define SMS_TP_UDHI_HEADER 0x40

static struct k_thread modem_rx_thread;
static struct k_work_q modem_workq;
static struct sim7000_data mdata;
 ...
 */
static void change_state(enum sim7000_state state)
{
 ...
 */
static enum sim7000_state get_state(void)
{
	return mdata.state;
}
...
static int offload_connect(void *obj, const struct sockaddr *addr, socklen_t addrlen)
{
	...
	if (get_state() != SIM7000_STATE_NETWORKING) {
		return -EAGAIN;
	}
...
int mdm_sim7000_query_gnss(struct sim7000_gnss_data *data)
{
	...
	if (get_state() != SIM7000_STATE_GNSS) {
...
int mdm_sim7000_start_gnss(void)
{
	...
	change_state(SIM7000_STATE_GNSS);
	return 0;
}
...
static int modem_setup(void)
{
	...
	change_state(SIM7000_STATE_NETWORKING);
...
int mdm_sim7000_power_off(void)
{
	...
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL, 0U, "AT", &mdata.sem_response,
			     K_MSEC(1000));
	if (ret < 0) {
		change_state(SIM7000_STATE_OFF);
		return 0;
	}
